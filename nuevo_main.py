...
CFG = {
    # AOI: ahora lo generaremos a partir de las coordenadas del target
    "aoi_geojson_url": None,
    "aoi_geojson_local": None,

    # Coordenadas del target de calibración (Shadnagar, India)
    "target_lat": 17.034261,
    "target_lon": 78.183078,

    "stac_url": "https://earth-search.aws.element84.com/v1",
    "collection": "sentinel-2-l2a",

    # Rango de fechas amplio para encontrar muchas escenas (ajusta si el profe dice otra cosa)
    "date_range": ["2017-01-01", "2025-01-01"],  # [inicio, fin)

    "bands": ["red", "green", "blue", "nir"],

    # Filtro de nubes (puedes bajar a 5.0 si quieres aún más limpio)
    "max_cloud_percent": 10.0,

    # Descargas y salidas
    "out_dir": "outputs_session18",      # si quieres separarlo de session5
    ...
    # Concurrencia
    "download_threads_baseline": 1,
    "download_threads_fast": 4,
    "ndwi_processes": 4,

    # Para extrapolaciones (da un poco igual aquí)
    "months_for_extrapolation": 6,
}
...
def load_aoi(cfg: Dict[str, Any]) -> Dict[str, Any]:
    """Carga AOI desde URL, fichero local, o genera un cuadrado pequeño alrededor del target."""
    # 1) Si hay geojson local, úsalo
    if cfg.get("aoi_geojson_local"):
        with open(cfg["aoi_geojson_local"], "r", encoding="utf-8") as f:
            return json.load(f)

    # 2) Si hay URL, úsala
    if cfg.get("aoi_geojson_url"):
        r = requests.get(cfg["aoi_geojson_url"], timeout=30)
        r.raise_for_status()
        return r.json()

    # 3) Si no hay nada, generamos un pequeño cuadrado alrededor del target
    lat = cfg["target_lat"]
    lon = cfg["target_lon"]

    # ~100 m en grados (aprox 0.001º son unos 100-110 m)
    delta = 0.001
...
def main():
    ...
    print("   Por satélite (filtrados):", summarize_by_platform(items_ok))
     # Ordenamos por fecha y nos quedamos con las 10 primeras escenas “buenas”
    items_ok = sorted(
        items_ok,
        key=lambda it: it.get("properties", {}).get("datetime", "")
    )
    items_ok = items_ok[:10]
    print(f"   Usando solo {len(items_ok)} escenas filtradas para la práctica (10 máx).")
